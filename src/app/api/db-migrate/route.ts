...
async function addProductsToDatabase() {
    for (const product of products) {
        ...
        // If category exists, create the product with the retrieved category ID
        if (category) {
            ...
            console.error(`Category "${product.category.name}" not found for product "${product.title}"`);
        }
    }
}
...
export const POST = async () => {
    try {
        const createdProducts = await Promise.all(products.slice(90, 100).map(async (product) => {
            const category = await prisma.category.findFirst({ where: {
                name: product.category.name
            }})

            const title = await prisma.product.create({
                data: {
                    title: product.title,
