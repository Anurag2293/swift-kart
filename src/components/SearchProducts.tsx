...
export default function SearchProducts({ queryBaseUrl }: { queryBaseUrl: string }) {
	...
	const { isPending, error, data, refetch: refetchProducts } = useQuery({
		...
		queryFn: () =>
			axios
				.get(`${queryBaseUrl}?take=${PRODUCTS_PER_PAGE}&skip=${(page - 1) * PRODUCTS_PER_PAGE}&sort=${sort}`)
				.then((res) => res.data)
	})

	useEffect(() => {
		refetchProducts()
	}, [page, sort, refetchProducts])
